...
def get_flow_stats(switch):
    result = subprocess.check_output(['ovs-ofctl', '-O', 'OpenFlow13', 'dump-flows', switch])
    print(result.decode())
...
def add_openflow_rules(switch):
    ...
    flow_rules = [
        ...
        # Extremely high priority for sensor data (using a specific port or protocol)
        'priority=40000,tcp,tp_dst=5555,actions=set_queue:2,output:NORMAL',

        
        # Moderate priority for sensor data control messages
        'priority=35000,udp,tp_dst=5556,actions=set_queue:1,output:NORMAL',
        
        # Low priority for regular traffic
        'priority=10000,ip,actions=set_queue:0,output:NORMAL',
        
        # Table-miss flow entry
        'priority=65535,actions=CONTROLLER:65535'
